...
class perceptron(object):
    ...
    def train(self,X,Y):
        ...
        while iteration < len(X):
            ...
            yin = self.bias + np.sum(np.dot(x,self.weight))                        #(1,2).(2,1)
            self.weight = self.weight + lr_rate*(np.dot(x.T,(target-yin)))         #(2,1)+(2,1).(1,1)=(2,1)
            self.bias = self.bias + lr_rate*(target - yin)                         #(1,1)+(1,1)=(1,1)
            iteration += 1
